...
#
# Parallel to build.sh (which targets v2.6.0 LTS).
# Builds into build_v33/ to avoid clobbering the v2.6 build artifacts.
...
CMAKE_BUILD_PARALLEL_LEVEL=16 west build -b nrf52840dk/nrf52840 -d build_v33 -- "$@"

UF2_FILE="${WORKSPACE_DIR}/build_v33/zephyr/zephyr.uf2"
if [ -f "$UF2_FILE" ]; then
    UF2_SIZE=$(stat -c%s "$UF2_FILE")
    if [ "$UF2_SIZE" -gt 1000000 ]; then
        echo "WARNING: UF2 too large (${UF2_SIZE} > 1000000)! flash.sh will refuse to flash."
    elif [ "$UF2_SIZE" -gt 900000 ]; then
        echo "WARNING: UF2 approaching limit (${UF2_SIZE}/1000000 bytes)"
    fi
fi

echo "Build Completed (build_v33/)"
